...
class EthereumChainService extends ethReader_1.EthereumChainReader {
    constructor(store, chainProviders, signer, log, defaultRetries = 1) {
        ...
        this.evts = {
            [vector_types_1.EngineEvents.TRANSACTION_SUBMITTED]: new evt_1.Evt(),
            [vector_types_1.EngineEvents.TRANSACTION_MINED]: new evt_1.Evt(),
            [vector_types_1.EngineEvents.TRANSACTION_FAILED]: new evt_1.Evt(),
        };
    ...
    async sendDeployChannelTx(channelState, gasPrice, deposit) {
        ...
        if (!deposit) {
            ...
            const result = await this.sendTxWithRetries(channelState.channelAddress, vector_types_1.TransactionReason.deploy, async () => {
                ...
                const _gas = vector_types_1.GAS_ESTIMATES.createChannel;
        ...
        if (assetId === constants_1.AddressZero) {
            return this.sendTxWithRetries(channelState.channelAddress, vector_types_1.TransactionReason.deployWithDepositAlice, async () => {
                ...
                const _gas = vector_types_1.GAS_ESTIMATES.createChannelAndDepositAlice;
                const gas = _gas.add(exports.EXTRA_GAS);
        ...
        const approveRes = await this.approveTokens(channelState.channelAddress, channelState.networkContext.channelFactoryAddress, sender, amount, assetId, channelState.networkContext.chainId, gasPrice);
    ...
    async sendWithdrawTx(channelState, minTx) {
        ...
        return this.sendTxWithRetries(channelState.channelAddress, vector_types_1.TransactionReason.withdraw, async () => {
            const _gas = vector_types_1.GAS_ESTIMATES.withdraw;
            const gas = _gas.add(exports.EXTRA_GAS);
    ...
    async sendDepositTx(channelState, sender, amount, assetId) {
        ...
        const toDeposit = bignumber_1.BigNumber.from(amount);
        if (toDeposit.isNegative()) {
            return vector_types_1.Result.fail(new vector_types_1.ChainError(vector_types_1.ChainError.reasons.NegativeDepositAmount));
        }
        const multisigRes = await this.getCode(channelState.channelAddress, channelState.networkContext.chainId);
        if (multisigRes.isError) {
            return vector_types_1.Result.fail(multisigRes.getError());
    ...
    async sendTxAndParseResponse(channelAddress, reason, txFn) {
        try {
            const response = await this.queue.add(async () => {
                ...
                this.evts[vector_types_1.EngineEvents.TRANSACTION_SUBMITTED].post({
                    response: Object.fromEntries(Object.entries(response).map(([key, value]) => {
                        return [key, bignumber_1.BigNumber.isBigNumber(value) ? value.toString() : value];
                    })),
                    channelAddress,
                    reason,
                });
                response
                    ...
                    if (receipt.status === 0) {
                        ...
                        this.evts[vector_types_1.EngineEvents.TRANSACTION_FAILED].post({
                            receipt: Object.fromEntries(Object.entries(receipt).map(([key, value]) => {
                                return [key, bignumber_1.BigNumber.isBigNumber(value) ? value.toString() : value];
                            })),
                            channelAddress,
                            reason,
                        ...
                        this.evts[vector_types_1.EngineEvents.TRANSACTION_MINED].post({
                            receipt: Object.fromEntries(Object.entries(receipt).map(([key, value]) => {
                                return [key, bignumber_1.BigNumber.isBigNumber(value) ? value.toString() : value];
                            })),
                            channelAddress,
    ...
    async approveTokens(channelAddress, spender, owner, depositAmount, assetId, chainId, gasPrice, approvalAmount = vector_types_1.UINT_MAX) {
        var _a, _b;
        ...
        if (allowanceRes.isError) {
            this.log.error({
                ...
                owner,
                assetId,
                error: (_a = allowanceRes.getError()) === null || _a === void 0 ? void 0 : _a.message,
            }, "Error checking approved tokens for deposit A");
            return vector_types_1.Result.fail(allowanceRes.getError());
        }
        const allowance = allowanceRes.getValue();
        this.log.info({ method, assetId, spender, owner, channelAddress, allowance: allowance.toString(), depositAmount }, "Retrieved allowance");
        if (bignumber_1.BigNumber.from(allowanceRes.getValue()).gte(depositAmount)) {
            this.log.info({
                method,
                assetId,
                channelAddress,
        ...
        const approveRes = await this.sendTxWithRetries(channelAddress, vector_types_1.TransactionReason.approveTokens, () => erc20.approve(spender, approvalAmount, { gasPrice }));
        if (approveRes.isError) {
            this.log.error({
                method,
                spender,
    ...
    async sendDepositATx(channelState, amount, assetId, gasPrice) {
        ...
        if (assetId !== constants_1.AddressZero) {
            ...
            const approveRes = await this.approveTokens(channelState.channelAddress, channelState.channelAddress, channelState.alice, amount, assetId, channelState.networkContext.chainId, gasPrice);
            if (approveRes.isError) {
                this.log.error({
                    method,
                    methodId,
