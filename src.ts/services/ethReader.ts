...
class EthereumChainReader {
    ...
    async getTotalDepositedA(channelAddress, chainId, assetId) {
        ...
        if (code.getValue() === "0x") {
            return vector_types_1.Result.ok(bignumber_1.BigNumber.from(0));
        }
    ...
    async getGasPrice(chainId) {
        ...
        if (chainId === 1) {
            try {
                ...
                gasPrice = typeof rapid !== "undefined" ? bignumber_1.BigNumber.from(rapid) : undefined;
            }
            catch (e) {
    ...
    async getOnchainBalance(assetId, balanceOf, chainId) {
        ...
        try {
            const onchainBalance = assetId === constants_1.AddressZero
    ...
    async getDecimals(assetId, chainId) {
        const provider = this.chainProviders[chainId];
        if (!provider) {
            return vector_types_1.Result.fail(new vector_types_1.ChainError(vector_types_1.ChainError.reasons.ProviderNotFound));
        }
        try {
            const decimals = assetId === constants_1.AddressZero ? 18 : await new contracts_1.Contract(assetId, vector_types_1.ERC20Abi, provider).decimals();
            return vector_types_1.Result.ok(decimals);
        }
        catch (e) {
            return vector_types_1.Result.fail(e);
        }
    }
    async getWithdrawalTransactionRecord(withdrawData, channelAddress, chainId) {
        const provider = this.chainProviders[chainId];
        if (!provider) {
            return vector_types_1.Result.fail(new vector_types_1.ChainError(vector_types_1.ChainError.reasons.ProviderNotFound));
        }
        ...
        if (code.getValue() === "0x") {
            return vector_types_1.Result.ok(false);
        }
        const channel = new contracts_1.Contract(channelAddress, artifacts_1.VectorChannel.abi, provider);
        try {
            ...
            return vector_types_1.Result.ok(record);
        }
        catch (e) {
            return vector_types_1.Result.fail(e);
        }
